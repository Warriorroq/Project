...
namespace Project.Game
{
    public class Game : GameLoop
    {
        ...
        private void CreateAgar()
        {
            ...
            foodSpawner.Init(new Food(), .01f, SetRandomPosition, DestroyFoodObj);
            var a = new FoodBall();
            currentScene.Add(a);
            currentScene.Add(foodSpawner);
        }
        private void SetRandomPosition(GameObject obj)
            =>obj.position = Program.random.RandomVector(new Vector2i(20, 20), new Vector2i(2540, 1420));
        private void DestroyFoodObj(GameObject obj)
        {
            if (Food.foodCount > 100)
                obj?.Destroy();
        }
    }
}
